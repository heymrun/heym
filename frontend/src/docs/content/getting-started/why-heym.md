...
The [Board tab](../tabs/board-tab.md) is a built-in Kanban board where the columns themselves execute workflows. A card is not just a task—it is a persistent agentic job that carries context, conversation history, execution state, outputs, and workflow runs.
...
The [Playwright node](../nodes/playwright-node.md) is first-party browser automation on the canvas—no external scraping service or community package required.
