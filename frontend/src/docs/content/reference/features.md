...
The Cal.com Trigger node receives booking and meeting webhooks, verifies their HMAC-SHA256 signature with a `cal_trigger` credential, and starts the workflow. The separate [Cal.com node](../nodes/cal-node.md) uses a `cal_api` credential to list, create, update, and delete remote webhooks. The trigger exposes the optional top-level event name, complete event body, normalized payload, sanitized headers (including the Cal.com payload version), and trigger timestamp.
...
#### [Cal.com](../nodes/cal-node.md)

The Cal.com action node uses a `cal_api` credential to list, create, update, and delete Cal.com API v2 webhooks. Webhook IDs and request bodies support expressions and Agent tool autofill. Pair it with a [Cal.com Trigger](../nodes/cal-trigger-node.md) by copying the trigger URL into the webhook `subscriberUrl` and configuring the same signing secret on both sides.
