...
The AI Agent node is an [LLM](../nodes/llm-node.md) node with tool calling. It can run Python tools, call connected canvas nodes as tools, connect to [MCP](../tabs/mcp-tab.md) servers, use skills (instruction files and optional Python scripts), act as an orchestrator that delegates to other agent nodes, and call other workflows as tools. Output is available as `$nodeLabel.text`. An optional **Extra Body** toggle attaches a JSON object of provider-specific request parameters to every request in the tool-calling loop; it is off by default. An optional **Use Responses API** toggle routes the agent's model calls through the Responses API, which preserves the model's reasoning across tool calls so a reasoning model keeps its chain of thought through a multi-step loop; it is off by default and works with OpenAI and custom credentials.
...
The Decision node asks a decision model typed questions about the run's state and returns probabilities instead of prose. A **State** field carries what the model should read — a lone expression keeps its own type, so `$input` sends the object rather than its text — and a list of questions carries the judgments. Questions come in three shapes: **noul** answers whether a condition holds, **choice** picks one option from a set you define and returns the full distribution, and **score** rates the state along ordered levels and can land between them. Choice and score answers also carry a confidence, so a [Switch](../nodes/switch-node.md) can route on the answer while a [Condition](../nodes/condition-node.md) gates on how certain it was. **Generate with AI** drafts the state template and the question rows from a plain sentence, and **Custom request body** sends hand-written JSON for an endpoint whose contract differs. The provider response is passed through unchanged, and each call is recorded in [Traces](../tabs/traces-tab.md).

Needs a Decision Model credential; pairs naturally with [Switch](../nodes/switch-node.md), [Condition](../nodes/condition-node.md), and [Agent Node](../nodes/agent-node.md).
...
It covers every non-test top-level run: both execute endpoints, the [Portal](./portal.md), and every trigger, including [cron](../nodes/cron-node.md), the chat platform triggers, email, queues, [MCP](../tabs/mcp-tab.md), and dashboard widgets. With [Load Distribution](./cluster.md) enabled this is unchanged, including runs a worker executed: the failing node travels back with the result, and the error workflow is then distributed like any other run, so a node inside it that has to run on the main instance still does. Two cases are deliberately left out, because the workflow's own outcome is not yet known: a run no instance ever executed, and a wait that timed out while the run may still be going. It is also **not** triggered by manual canvas test runs.
