...
The Cal.com Trigger node receives signed Cal.com webhooks—booking, meeting, recording, routing-form, no-show, delegation, and related events—verifies their HMAC-SHA256 signature, and starts the workflow. Manual mode uses a selected `cal_trigger` credential. Managed mode uses the Cal.com API v2 contract through a `cal_api` credential to create, update, and disable the webhook, generate and encrypt its signing secret, select events from the managed allowlist, choose the `2021-10-20` payload schema or its backward-compatible `2026-07-27` ICS extension, and configure Cal Video no-show evaluation delays in Heym. The node exposes the optional top-level event name, complete event body, normalized payload, sanitized headers (including the Cal.com payload version), and trigger timestamp.
