...
> **Team sharing:** `shareWithMyTeams` creates explicit shares for the owner's current teams. Future teams are not added automatically unless the operation runs again. `unshareWithMyTeams` removes all team shares from the file.
...
- `$nodeLabel.status` — `"success"`
- `$nodeLabel.file_id` — UUID of the file
- `$nodeLabel.shared_team_count` — `0` after the operation
