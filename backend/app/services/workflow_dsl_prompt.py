...
- **Purpose**: List and manage files generated by skills — retrieve metadata, delete, password-protect, set expiry (TTL), limit downloads, or share with teams
- **Inputs**: 1 | **Outputs**: 1
- **No credential required** — operates on files owned by the workflow owner
- **Data fields**:
  - `label`: Node identifier
  - `driveOperation`: Operation — `"get"` | `"getAll"` | `"downloadUrl"` | `"save"` | `"convertFile"` | `"delete"` | `"setPassword"` | `"setTtl"` | `"setMaxDownloads"` | `"shareWithMyTeams"`
...
- `$nodeLabel.status` - `"deleted"` (delete), `"updated"` (set* operations), or `"success"` (shareWithMyTeams)
...
- `$nodeLabel.shared_team_count` - Number of teams the file is shared with (shareWithMyTeams only)
